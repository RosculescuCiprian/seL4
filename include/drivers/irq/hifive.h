...
{
    return PLAT_PLIC_EN_ADJUST(PLIC_EN + hart_id * PLIC_EN_PER_HART + context_id * PLIC_EN_PER_CONTEXT);
}

static inline word_t plic_thres_offset(word_t hart_id, word_t context_id)
{
    return PLAT_PLIC_EN_ADJUST(PLIC_THRES + hart_id * PLIC_THRES_PER_HART + context_id * PLIC_THRES_PER_CONTEXT);
}

static inline word_t plic_claim_offset(word_t hart_id, word_t context_id)
{
    return plic_thres_offset(hart_id, context_id) + PLIC_THRES_CLAIM;
}
...
static inline interrupt_t plic_get_claim(void)
{
    ...
    return readl(PLIC_PPTR_BASE + plic_claim_offset(PLIC_HART_ID, PLIC_SVC_CONTEXT));
}

static inline void plic_complete_claim(interrupt_t irq)
{
    /* Complete the IRQ claim by writing back to the claim register. */
    writel(irq, PLIC_PPTR_BASE + plic_claim_offset(PLIC_HART_ID, PLIC_SVC_CONTEXT));
}
...
static inline void plic_mask_irq(bool_t disable, interrupt_t irq)
{
    ...
    addr = PLIC_PPTR_BASE + plic_enable_offset(PLIC_HART_ID, PLIC_SVC_CONTEXT) + (irq / 32) * 4;
    bit = irq % 32;
